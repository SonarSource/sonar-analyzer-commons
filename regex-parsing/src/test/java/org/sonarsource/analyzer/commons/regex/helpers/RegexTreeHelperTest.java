...
import static org.sonarsource.analyzer.commons.regex.helpers.SimplifiedRegexCharacterClassTest.parseRegex;
...
class RegexTreeHelperTest {
  ...
  @Test
  void only_matches_empty_suffix() {
    assertThat(RegexTreeHelper.onlyMatchesEmptySuffix(parseRegex("hello", new FlagSet(0)).getResult())).isFalse();
    assertThat(RegexTreeHelper.onlyMatchesEmptySuffix(parseRegex("hello", new FlagSet(0)).getResult().continuation())).isTrue();
    assertThat(RegexTreeHelper.onlyMatchesEmptySuffix(parseRegex(".*?((?=))*", new FlagSet(0)).getResult())).isFalse();
  }
